...
# Configuration (adjustable per location)
...
# Calculate actual model size from disk
cache_dir = Path.home() / ".cache" / "huggingface" / "hub"
model_dirs = list(cache_dir.glob("models--black-forest-labs--FLUX.1-schnell"))
model_size_gb = 0
if model_dirs:
...
# Energy estimation based on CPU TDP
energy_kwh = (CPU_TDP_WATTS * generation_time / 3600) / 1000

# CO2 estimation based on grid intensity
...
print(f"\nNote: Energy/CO2 are estimates. Actual values depend on:")
print(f"  - Real CPU power draw (varies with load)")
print(f"  - Local electricity grid carbon intensity")
print("\nBanner saved")
